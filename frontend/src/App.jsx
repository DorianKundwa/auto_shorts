...
const DESTINATIONS = [
  ...
  { id: 'YouTube',   name: 'YouTube',   ratio: '9:16' },
];

// ─── Helper ──────────────────────────────────────────────────────────────────
// Clips stored by the new pipeline are {path, title} objects.
// The DB normaliser also converts old plain-string paths to {path, title: null}.
...
function App() {
  ...
  // ── Polling — keyed on jobId only, clears itself on terminal state ────────
  useEffect(() => {
    if (!jobId) return;
      ...
      <div className="container mx-auto px-6 py-16 max-w-6xl relative z-10">
        ...
        <header className="text-center mb-16">
          ...
          <p className="text-slate-400 text-xl max-w-2xl mx-auto leading-relaxed">
            Upload your long-form video and instantly generate viral hooks perfectly formatted for any social platform.
          </p>
        </header>
          ...
          <div className="lg:col-span-5 space-y-6">
            {!jobId ? (
              ...
              >
                ...
                <div className="mb-8">
                  <h3 className="text-lg font-semibold mb-4 flex items-center gap-2 text-slate-200">
                    <Film className="w-5 h-5 text-yellow-400" /> Number of Clips
                  </h3>
                  <div className="flex items-center gap-4">
                    ...
                    <span className="text-slate-500 text-sm">hooks extracted (1–5)</span>
                  </div>
                </div>
                  ...
                  <div className="w-full border-2 border-dashed border-slate-700 hover:border-slate-500 hover:bg-slate-800/50 rounded-2xl p-6 text-center cursor-pointer transition-all duration-300 relative overflow-hidden">
                    ...
                    {transcriptFile ? (
                      ...
                      <>
                        ...
                        <p className="text-slate-500 text-xs mt-1">Skips Whisper AI — much faster</p>
                      </>
                    )}
                  </div>
                </div>
                ...
                {error && (
                  <motion.p
                    ...
                    className="text-red-400 mt-4 text-center font-medium text-sm bg-red-400/10 py-2 rounded-lg border border-red-400/20"
                  >
                    {error}
                  </motion.p>
                )}
              ...
              >
                ...
                <div className="absolute top-0 left-0 w-full h-1 bg-slate-800">
                  <motion.div
                    ...
                    initial={{ width: 0 }}
                    animate={{ width: `${jobStatus?.progress || uploadProgress || 0}%` }}
                ...
                <div className="space-y-6">
                  <div className="flex justify-between items-center text-sm">
                    ...
                    <span className="font-bold text-slate-300 bg-slate-800 px-3 py-1 rounded-full">
                      {jobStatus?.progress || uploadProgress || 0}%
                    </span>
                  </div>
                  ...
                  {jobStatus?.status !== 'completed' && jobStatus?.status !== 'failed' && (
                    <div className="mt-8 p-4 bg-slate-800/50 border border-slate-700 rounded-xl">
                      <p className="text-slate-400 text-sm flex items-center gap-2">
                        ...
                        Extracting viral hooks, detecting faces, and burning karaoke captions...
                      </p>
                    </div>
                  )}
                  ...
                  {(jobStatus?.status === 'completed' || jobStatus?.status === 'failed') && (
                    <motion.button
                      ...
                      onClick={handleReset}
                      className="w-full py-3 rounded-xl font-semibold text-sm flex items-center justify-center gap-2 bg-slate-800 hover:bg-slate-700 border border-slate-700 hover:border-slate-500 text-slate-300 transition-all duration-200"
                    >
              ...
              >
                <AnimatePresence>
                  {jobStatus.clips.map((clip, i) => {
                    ...
                    // Extract platform badge from last underscore segment before extension
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="p-5 bg-slate-900">
                          {/* Show LLM-generated title when available, else generic fallback */}
                          <h4 className="font-bold text-lg text-slate-200 mb-1 truncate" title={title ?? `Generated Short #${i + 1}`}>
